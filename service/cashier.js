...
export const requestWithdraw = async (amount, ticker, address) => {
  return await authPost(`${BASE_URL}/withdraw`, SEC_TOKEN, {
    amt: amount,
...
export const prepareOrder = async (MemberId, numbers) => {
  return await authPost(`${BASE_URL}/prepare-order`, SEC_TOKEN, {
    MemberId,
    BrandId: BRAND_ID,
    ProductNumsLottery: numbers,
  });
};
...
export const confirmProcessorOrder = async (
  ...
  return await authPost(`${BASE_URL}/processor-confirm-order`, SEC_TOKEN, {
    PhoneOrEmail: email,
    ProcessorApi: "",
    AffiliatedId: 0,
    ReedemCode: "",
...
export const getMemberPaymentMethods = async () => {
  return await authPost(`${BASE_URL}/get-member-payment-methods`, SEC_TOKEN);
};

export const depositFunds = async () => {
  return await authPost(`${BASE_URL}/deposit-funds`, SEC_TOKEN);
};
...
export const confirmProcessorDeposit = async (MemberId, Amount, Ticker) => {
  return await authPost(`${BASE_URL}/processor-confirm-deposit`, SEC_TOKEN, {
    BrandId: BRAND_ID,
    MemberId,
    Amount,
    Ticker,
...
export const vouchersDeposit = async () => {
  return await authPost(`${BASE_URL}/CreditVouchers-deposit`, SEC_TOKEN);
};
