        ...
        <div className="wrap">
          <div id="middle" className="innerbg">
            <div className="groups-page">
              <h1>
                Lottery Groups • Get more numbers • Better odds of winning
              </h1>
              ...
              <div className="all-groups">
                {draws &&
                  draws.map((draw) => (
                    <article
                      className={`group-item ${draw.name.toLowerCase()}`}
                      key={`${draw.type}-${draw.id}`}
                    >
                      <Link href={draw.link}>
                        <div style={{ cursor: "pointer" }}>
                          <div className="group-box">
                            ...
                            <div className="group-jackpot">
                              {draw.amount < 0
                                ? "PENDING"
                                : `${draw.unit} ${formatNumber(draw.amount)}`}
                            </div>
                          </div>
                          <div className="group-middle">
                            <strong>50</strong> Lines <strong>150</strong>{" "}
                            Shares
                          </div>
                        </div>
                      </Link>
              ...
              <div className="resultschecker">
                ...
                <h1 className="item-title">
                  Boost Your Chances With A Group Acquisition
                </h1>
                <p>
                  We now have the offer that enables you to join and play in
                  traditional lottery groups. Each member started a group for
                  their own lottery with up to 150 vacant seats for each draw.
                  As long as there are vacant seats, anyone can join anytime and
                  play with 50 lines per draw. You can instantly boost your
                  winning chances to any lotto game you choose. The group owners
                  selected themselves the lucky numbers for each lottery, taking
                  into consideration that some lotteries have extra bonus
                  numbers. For those special lotteries, all potential
                  combinations of the extra bonus numbers are picked so there is
                  a guaranteed win on each draw.
                </p>
                <p>&nbsp;</p>
                <p>&nbsp;</p>
                <h1 className="item-title">
...
export const getStaticProps = async (ctx) => {
  try {
    ...
    const draws = res[0]
      ...
      .filter(
        (draw) =>
          draw.LotteryTypeId !== 45 &&
          draw.LotteryTypeId !== 46 &&
