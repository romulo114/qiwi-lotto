...
const LottoResult = ({ data }) => {
  ...
  useEffect(() => {
    ...
    if (result) {
      ...
      const prizes = data.prizes.filter(
        (item) => item.DrawId === result.DrawId
      );
...
export async function getStaticPaths() {
  ...
  const names = lotteries
    .filter((lotto) => uniqueIds.includes(lotto.LotteryTypeId))
...
export async function getStaticProps(context) {
  ...
  try {
    ...
    const results = allResults.filter(
      (res) => res.LotteryTypeId === lottery.LotteryTypeId
    );
    const prizes = result[2].filter(
      (prize) => prize.LotteryTypeId === lottery.LotteryTypeId
    );
