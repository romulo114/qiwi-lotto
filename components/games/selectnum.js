...
const SelectNumbers = ({ data, numTickets, onSelected, selE, selM }) => {
  ...
  const quickPick = useCallback(
    (e) => {
      const ms = randomArray(
        1,
        data.NumberOfMainNumbers,
        data.AmountOfMainNumbersToMatch
      );
      const es = randomArray(
        1,
        data.NumberOfExtraNumbers,
    ...
    <div className="select_num_col">
      <div className="select_num_col_part">
        ...
        <div className="quickpic">
          <div className="quickpic_text on_ticket" onClick={quickPick}>
            &nbsp;
          </div>
          <h6 className="pick_num_title">{`Pick ${data.AmountOfMainNumbersToMatch} Numbers`}</h6>
          <a className="quickpic_delete" onClick={deletePick}></a>
        ...
        <div className="all_num_part">
          ...
          <div className="lt_numbers_wrapper">
            {generateArray(1, data.NumberOfMainNumbers).map((i) => (
              <span
                key={i}
                className={selM.includes(i) ? "main_active" : ""}
                onClick={handleMain}
              >
                {i}
              </span>
        ...
        {data.NumberOfExtraNumbers > 0 && (
          <div className="select_num_part">
            <h5 className="slide-trigger">{`Pick ${
              lotteriesNames[data.LotteryName]
            }`}</h5>
            <div className="select_num_part_wrapper">
              {generateArray(
                ...
                <span
                  className={selE.includes(i) ? "extra_active line" : "line"}
                  key={i}
                  onClick={handleExtra}
                >
                  {i}
