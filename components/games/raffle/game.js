...
const RaffleTickets = ({ tickets, draw }) => {
  ...
  const enoughBalance =
    balance?.AccountBalance >= selections.length * draw.price;
  const handleContinue = useCallback(
    async (e) => {
      ...
      try {
        setBusy(true);
        const result = await confirmRaffleOrder(
          profile.MemberId,
          currency,
          draw.type,
          selections
      ...
      <div id="btcowl-demo" className="owl-carousel">
        {tickets.map((ticket) => (
          <div
            ...
            onClick={() => toggleSelections(ticket.Id)}
          >
            <div className="flexRow">
              <label className="cb-container">
                <input
                  type="checkbox"
                  checked={selections.indexOf(ticket.Id) >= 0}
                  readOnly
                />
            ...
            <div className="play-now">
              ...
              <span className="ticket-price">
                Ticket Price: €{parseInt(draw.price)}
              </span>
            </div>
          </div>
        ))}
