...
const GroupGame = (props) => {
  ...
  const discount = (
    ((shares * draws * data.PricePerShare) / 8) *
      data.Options.find((opt) => opt.NumberOfDraws === draws)?.Discount ?? 0
  ).toFixed(2);
        ...
        <section className="body-section">
          ...
          <div className="body-right">
            ...
            <p className="hidden-xs">
              Increase your winning chances with a Group Subscription. 1 in 4
              jackpots is won by a Group Ticket. On this Subscription we cover
              all the bonus numbers to increase the winning chances of the
              group. You get: 50 lines and up to 150 shares in the group. Get
              more shares so you'll have a bigger portion from the winning
            </p>
        ...
        <section className="group-option-section">
          <div className="choose-option">
              ...
              <TTInput
                ...
                tooltip={
                  <span style={{ fontSize: 14 }}>
                    ...
                    <span>
                      1 Draw <hr /> <br />
                      Play for the next upcoming Draw only. <br /> Try a
                      Multi-Draw or Subscription and get higher discounts per
                      draw.
                    </span>
                  </span>
                }
            ...
            <div className="draw-option">
              <TTInput
                ...
                id="multi_drawop"
                className="css-checkbox"
                desc={<span style={{ fontSize: 16 }}>Multi-Draw</span>}
                checked={option === 1}
                ...
                tooltip={
                  <span style={{ fontSize: 14 }}>
                    ...
                    <span>
                      Multi-draw <hr />
                      <br />
                      Choose Multi-Draw to play for several draws in advance.
                      Save up to 20% per draw.
                    </span>
                  </span>
                }
              />
              {option === 1 && (
                <select
                  className="group_totaldraw"
                  name="group_totaldraw"
                  value={draws}
                  onChange={(e) => setDraws(parseInt(e.target.value))}
                >
                  {options.map((opt) => (
                    <option
                      value={opt.NumberOfDraws}
                      key={`${data.LotteryName}-${opt.NumberOfDraws}`}
                    >
                      {parseInt(opt.Discount * 100) === 0
                        ? `${opt.NumberOfDraws} draws`
                        : `${opt.NumberOfDraws} draws ${parseInt(
                            opt.Discount * 100
                          )} % discount`}
                    </option>
                  ))}
          ...
          <div className="total-sum">
            ...
            <div className="total">
              ...
              <span className="total-price">{`€ ${(total - discount).toFixed(
                2
              )}`}</span>
            </div>
            <button
              className="oro-single-total_share_conti_btn"
              onClick={gotoCart}
            >
              Continue
            </button>
